...
export default class GameScene extends Phaser.Scene {
  ...
  update() {
    ...
    if (jump && onGround) { this.player.setVelocityY(jumpVel); this.playSfx('jump'); }
  }
  ...
  loadLevel(index) {
    ...
    for (const p of lvl.platforms) this.platforms.create(p.x, p.y, p.key).refreshBody();

    this.stars = this.physics.add.group({ key: 'star', repeat: lvl.stars.count - 1, setXY: { x: lvl.stars.startX, y: 0, stepX: lvl.stars.stepX } });
    this.stars.children.iterate((child) => { child.setBounceY(Phaser.Math.FloatBetween(0.2, 0.6)); });
    this.physics.add.collider(this.stars, this.platforms);
    this.physics.add.overlap(this.player, this.stars, this.collectStar, null, this);
  ...
  makeToneBuffer(freq, duration, type = 'sine', opts = {}) {
    const ctx = this.audioCtx; const sampleRate = ctx.sampleRate; const length = Math.floor(sampleRate * duration);
    const buffer = ctx.createBuffer(1, length, sampleRate); const data = buffer.getChannelData(0);
    const slideTo = opts.slideTo || freq;
    for (let i = 0; i < length; i++) {
      const t = i / sampleRate; const f = freq + (slideTo - freq) * (i / length);
      let v; const w = 2 * Math.PI * f * t;
      switch (type) { case 'square': v = Math.sign(Math.sin(w)); break; case 'triangle': v = 2/Math.PI * Math.asin(Math.sin(w)); break; case 'saw': v = (2 * (t * f - Math.floor(0.5 + t * f))); break; default: v = Math.sin(w); }
  ...
  playSfx(name) {
    ...
    const ctx = this.audioCtx; const src = ctx.createBufferSource(); src.buffer = this.buffers[name];
    const gain = ctx.createGain(); gain.gain.value = 0.8; src.connect(gain).connect(ctx.destination); src.start();
  }

  playMusic() {
    if (this.muted) return; const ctx = this.audioCtx; const tempo = 120; const beat = 60 / tempo; const seq = [523, 659, 784, 659, 523, 659, 784, 880];
    const osc = ctx.createOscillator(); const gain = ctx.createGain(); const filter = ctx.createBiquadFilter();
    osc.type = 'sine'; filter.type = 'lowpass'; filter.frequency.value = 1200; gain.gain.value = 0.06; osc.connect(filter).connect(gain).connect(ctx.destination);
    const start = ctx.currentTime + 0.05; seq.forEach((f, i) => { const t = start + i * beat * 0.5; osc.frequency.setValueAtTime(f, t); });
    const loopDur = seq.length * beat * 0.5; let loopCount = 0;
    const scheduleLoop = () => { if (this.gameOver || this.muted) return; const base = ctx.currentTime + 0.05; seq.forEach((f, i) => { const t = base + i * beat * 0.5; osc.frequency.setValueAtTime(f, t); }); loopCount += 1; this.time.delayedCall(loopDur * 1000, scheduleLoop); };
    osc.start(start); this.time.delayedCall(loopDur * 1000, scheduleLoop); this.music = { osc, gain };
  }

  stopMusic() { if (this.music?.osc) { try { this.music.osc.stop(); } catch (_) {} } this.music = null; }
  toggleMute() { this.muted = !this.muted; if (this.muted) this.stopMusic(); else this.playMusic(); this.brandText.setText(this.muted ? 'Sua Marca • Mudo' : 'Sua Marca'); }
}
