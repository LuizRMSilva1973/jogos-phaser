export default class BootScene extends Phaser.Scene {
  ...
  preload() {
    // Nada a carregar: texturas geradas por código
  }
  ...
  createGeneratedTextures() {
    ...
    for (let x = 0; x < 64; x += 8) g.fillRect(x, 16, 4, 16);
    ...
    g.beginPath(); g.moveTo(cx, cy - r1);
    for (let i = 0; i < spikes; i++) {
      x = cx + Math.cos(rot) * r1; y = cy + Math.sin(rot) * r1; g.lineTo(x, y);
      rot += Math.PI / spikes;
      x = cx + Math.cos(rot) * r2; y = cy + Math.sin(rot) * r2; g.lineTo(x, y);
      rot += Math.PI / spikes;
    }
    g.lineTo(cx, cy - r1); g.closePath(); g.fillPath();
    g.generateTexture('star', starSize, starSize);

    // Bomb (20x20)
    g.clear(); g.fillStyle(0xff4d4f, 1); g.fillCircle(10, 10, 10);
    ...
    g.beginPath(); g.moveTo(42, 20); g.lineTo(22, 32); g.lineTo(42, 44); g.closePath(); g.fillPath();
    g.generateTexture('icon-left', 64, 64);
    g.clear(); g.fillStyle(0xffffff, 0.9);
    g.beginPath(); g.moveTo(22, 20); g.lineTo(42, 32); g.lineTo(22, 44); g.closePath(); g.fillPath();
    g.generateTexture('icon-right', 64, 64);
    g.clear(); g.fillStyle(0xffffff, 0.9);
    g.beginPath(); g.moveTo(32, 18); g.lineTo(44, 40); g.lineTo(20, 40); g.closePath(); g.fillPath();
